...
En este caso no hay estacas que se salen (autocorrelaciones significativas) y por tanto loa valores son independientes e identicamente distribuidos. La ausencia de autocorrelaciones significativas de los valores al cuadrado o en términos absolutos reflejan la falta de agrupamiento de volatilidad. 
...
En terminos de medidas de ajuste se puede discernir que el mejor modelo es el AAR pues posee mejores indicadores tanto para MAE como para MAPE y RSME, seguido del modelo STAR. En el apartado gráfico se observa que, aunque similares, se puede concluir que el modelo que mas se acerca es el de AAR, pues al final del pronostico pues sigue la tendencia que lleva la serie original hacia la alta. 

```{r include=FALSE}

saveRDS(aar_mod, file = paste0(modelos, "/modelo_no_lineal_dolares.Rds"))
