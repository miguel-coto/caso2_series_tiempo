...
decomp_dolares = decompose(dolares_ts_full, "multiplicative")
...
dolares %>%
    ...
    annotate("text", x = ymd("2010-02-01"), y = 1000,
             color = palette_light()[[1]], label = "Región entrenamiento") +
    annotate("text", x = ymd("2020-03-01"), y = 200,
             color = palette_light()[[1]], label = "Región de \nprueba") +
    geom_point(alpha = 0.5, color = palette_light()[[1]]) +
    labs(title = "Bikes Sharing Dataset: Daily Scale", x = "", y="") +
    theme_tq()
```
...
test_tbl  <- dolares %>% filter(date >= ymd("2020-02-01"))
```
...
train_data <- bake(prep(recipe_spec_final), new_data = dolares) %>% 
   slice(1:(n()-6))

test_data <- bake(prep(recipe_spec_final), new_data = dolares)  %>% 
   slice((n()-5):n())
...
train_data_decomp <- bake(prep(recipe_spec_final), new_data = dolares) %>% 
...
test_data_decomp <- bake(prep(recipe_spec_final), new_data = dolares)  %>% 
  mutate(lag1 = lag(value,1), 
         lag3 = lag(value,3),
         lag6 = lag(value,6),
         lag12 = lag(value,12)) %>% 
...
```{r}
#bay_reg <- makeLearner("regr.bartMachine")
...
acc_todos <- tibble( Metodo = c("Glm","GBM","RF"),
        RMSE = round(c(
                       forecast::accuracy(pred_1, dolares_ts_val)[2],
                       forecast::accuracy(pred_2, dolares_ts_val)[2],
                       forecast::accuracy(pred_3, dolares_ts_val)[2]),3),
        MAE = round(c(
                       forecast::accuracy(pred_1, dolares_ts_val)[3],
                       forecast::accuracy(pred_2, dolares_ts_val)[3],
                       forecast::accuracy(pred_3, dolares_ts_val)[3]),3),
        MAPE = round(c(
                       forecast::accuracy(pred_1, dolares_ts_val)[5],
                       forecast::accuracy(pred_2, dolares_ts_val)[5],
                       forecast::accuracy(pred_3, dolares_ts_val)[5]),3)
...
saveRDS(glmnet_mod, file = paste0(modelos, "/modelo_ml_dolares.Rds"))
